...
export default function AccountingSuppliersPage() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse>({
    queryKey: ['supplier-debts', page, statusFilter],
    queryFn: () => {
      const params: Record<string, unknown> = { page, per_page: 20 }
      if (statusFilter) params.status = statusFilter
      return api.get('/accounting/suppliers', { params }).then(res => res.data)
    },
  })
  ...
  const columns = [
    {
      key: 'supplier_id',
      header: 'المورد',
      render: (item: Debt) => suppliers.find((s: Supplier) => s.id === item.supplier_id)?.name || '-',
    },
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="إدارة الحسابات الدائنة للموردين"
        action={
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <StatsCard title="إجمالي المبالغ المستحقة" value={totalPayable} icon={DollarSign} color="red" isCurrency />
      ...
      <Dialog isOpen={showDebt} onClose={() => setShowDebt(false)} title="إضافة دين جديد">
        <form onSubmit={handleCreateDebt} className="space-y-4">
          <div>
            ...
            <select value={debtForm.supplier_id} onChange={e => setDebtForm({ ...debtForm, supplier_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
          ...
          <div>
            ...
            <select value={paymentForm.debt_id} onChange={e => setPaymentForm({ ...paymentForm, debt_id: e.target.value })} className="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" required>
              ...
              {debts.filter((d: Debt) => d.status !== 'paid').map((d: Debt) => {
                const supName = suppliers.find((s: Supplier) => s.id === d.supplier_id)?.name || ''
                return <option key={d.id} value={d.id}>{supName} - {d.description} - متبقي {formatCurrency(d.remaining)}</option>
              })}
            </select>
          </div>
