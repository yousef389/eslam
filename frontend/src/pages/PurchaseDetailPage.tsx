...
export default function PurchaseDetailPage() {
  ...
  const statusMutation = useMutation({
    mutationFn: (newStatus: string) => api.put(`/purchase-orders/${id}/status`, { status: newStatus }),
    onSuccess: () => {
  ...
  const supplierName = suppliers.find((s: Supplier) => s.id === order.supplier_id)?.name || '-'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between"><span className="text-gray-500">المورد:</span><span className="font-medium">{supplierName}</span></div>
