...
import { ArrowRight, ChevronLeft, Printer, XCircle, Trash2 } from 'lucide-react'
...
export default function SaleDetailPage() {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/sale-orders/${id}`),
    onSuccess: () => {
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            ...
            <button onClick={() => { if(confirm('هل أنت متأكد من حذف الفاتورة نهائياً؟')) deleteMutation.mutate() }} disabled={deleteMutation.isPending} className="flex items-center gap-2 bg-red-800 text-white px-4 py-2 rounded-lg hover:bg-red-900 disabled:opacity-50">
              <Trash2 size={18} />حذف
            </button>
            {next && (
