...
from app.application.use_cases.accounting_use_cases import (
    CreateCustomerDebtUseCase,
    ListCustomerDebtsUseCase,
...
async def list_customer_debts(
    ...
    repo = CustomerDebtRepositoryImpl(db)
    use_case = ListCustomerDebtsUseCase(repo)
...
async def create_customer_debt(
    ...
    use_case = CreateCustomerDebtUseCase(debts_repo, customers_repo)
    ...
    return {"success": True, "data": _debt_to_dict(debt), "message": "Customer debt created"}
...
async def record_customer_payment(
    ...
    customers_repo = CustomerRepositoryImpl(db)
    use_case = RecordCustomerPaymentUseCase(debts_repo, payments_repo, customers_repo)
...
async def get_customer_debt(
    ...
    if not debt:
        raise NotFoundException("Customer debt", debt_id)
    payments_repo = DebtPaymentRepositoryImpl(db)
