...
from app.infrastructure.repositories.sale_order_item_repository import SaleOrderItemRepositoryImpl
...
async def get_sale_order(
    ...
    use_case = GetSaleOrderUseCase(orders_repo, items_repo)
...
async def create_sale_order(
    ...
    use_case = CreateSaleOrderUseCase(orders_repo, items_repo, products_repo, customers_repo)
    items_data = [
        {
            "product_id": item.product_id,
            "quantity": item.quantity,
    ...
    return {"success": True, "data": _order_to_dict(order), "message": "Sale order created"}
