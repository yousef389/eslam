...
from app.domain.entities import SaleOrderItem
from app.domain.repositories import SaleOrderItemRepository
from app.domain.value_objects import Money
from app.infrastructure.models.sale_orders import SaleOrderItemModel

from .base import BaseRepositoryImpl


def _model_to_entity(model: SaleOrderItemModel) -> SaleOrderItem:
...
def _entity_to_model(entity: SaleOrderItem) -> SaleOrderItemModel:
...
class SaleOrderItemRepositoryImpl(SaleOrderItemRepository):
    def __init__(self, session: AsyncSession) -> None:
        self._session = session
        self._repo = BaseRepositoryImpl(session, SaleOrderItemModel)

    async def get_by_id(self, id: str) -> Optional[SaleOrderItem]:
        model = await self._repo.get_by_id(id)
        return _model_to_entity(model) if model else None

    async def get_all(self, page: int = 1, per_page: int = 20) -> Tuple[List[SaleOrderItem], int]:
        models, total = await self._repo.get_all(page, per_page)
        return [_model_to_entity(m) for m in models], total

    async def create(self, entity: SaleOrderItem) -> SaleOrderItem:
    ...
    async def update(self, entity: SaleOrderItem) -> SaleOrderItem:
    ...
    async def get_by_order(
        ...
        count_stmt = (
            select(func.count())
            .select_from(SaleOrderItemModel)
            .where(SaleOrderItemModel.order_id == order_id)
        )
        total_result = await self._session.execute(count_stmt)
        total = total_result.scalar() or 0

        stmt = (
            select(SaleOrderItemModel)
            .where(SaleOrderItemModel.order_id == order_id)
